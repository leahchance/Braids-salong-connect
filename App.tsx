...
type PaymentResult = { ok: boolean; txId?: string; error?: string };

async function payDeposit(method: PaymentMethod, amountCents: number, bookingId: string): Promise<PaymentResult> {
  console.log("Deposit", method, amountCents, bookingId);
  await new Promise((r) => setTimeout(r, 900));
  return { ok: true, txId: "tx_dep_" + Date.now() };
}
async function payRemainder(method: PaymentMethod, amountCents: number, bookingId: string): Promise<PaymentResult> {
